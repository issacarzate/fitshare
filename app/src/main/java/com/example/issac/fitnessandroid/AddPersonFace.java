...
import static com.example.issac.fitnessandroid.AjustesActivity.imagen_galeria;

public class AddPersonFace extends AppCompatActivity {
    private ImageButton SelectPostImage;
    private Button UpdatePostButton;
    private EditText PostDescription;
    private ProgressDialog loadingBar;
    Intent intent = getIntent();
    public String idUser = intent.getStringExtra("id");
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_add_person_face);
        ...
        UpdatePostButton = (Button) findViewById(R.id.sendImage);
    ...
    private void GuardarImagenEnFirebaseStorage() {
        ...
        StorageReference filePath = ImagenRutinaReference.child("Imagenes Faces").child(ImageUri.getLastPathSegment() + enviarNombreRandom + ".jpg");
        ...
        filePath.putFile(ImageUri).addOnCompleteListener(new OnCompleteListener<UploadTask.TaskSnapshot>() {
            ...
            public void onComplete(@NonNull Task<UploadTask.TaskSnapshot> task) {
                if(task.isSuccessful()){
                    ...
                    postPreferences(UrlDescarga, idUser);

                    postPreferences(UrlDescarga, "22e75c84-182f-4f2a-b22b-235faabec4ab");
                    Toast.makeText(AddPersonFace.this, "Imagen Subida con Éxito", Toast.LENGTH_SHORT).show();
                    loadingBar.dismiss();
                }else{
                    String message = task.getException().getMessage();
                    Toast.makeText(AddPersonFace.this, "Erro: " + message, Toast.LENGTH_SHORT).show();
                    loadingBar.dismiss();
                }
            }
        });
    ...
    private void postPreferences (final String image, final String id){
        Thread thread = new Thread(new Runnable() {
            @Override
            public void run() {
                try {
                    URL url = new URL("http://hackweb.azurewebsites.net/api/uri");
